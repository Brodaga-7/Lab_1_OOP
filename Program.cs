...
namespace Lab1_SmartWatch
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            Console.WriteLine("Лабораторна робота №1. Варіант: Розумні годинники.");

            while (true)
            {
                // МЕНЮ ТОЧНО ЯК НА СКРІНШОТІ
                Console.WriteLine("\n--- МЕНЮ ---");
        ...
        static void PrintTable(List<SmartWatch> list)
        {
            ...
            Console.WriteLine("| {0,-3} | {1,-20} | {2,-10} | {3,-10} | {4,-15} | {5,-8} | {6,-12} |",
                "#", "Модель", "Ціна ($)", "Заряд(%)", "ОС", "Вода?", "Дата");
            Console.WriteLine(new string('-', 105));

            for (int i = 0; i < list.Count; i++)
            {
                var w = list[i];
                // Обрізка назви, якщо вона довша за 20 символів (для краси таблиці)
                string shortName = w.ModelName.Length > 20 ? w.ModelName.Substring(0, 17) + "..." : w.ModelName;

                Console.WriteLine("| {0,-3} | {1,-20} | {2,-10} | {3,-10} | {4,-15} | {5,-8} | {6,-12} |",
                    i + 1, shortName, w.Price, w.BatteryLevel, w.OsType, w.IsWaterproof ? "+" : "-", w.ManufactureDate.ToShortDateString());
            }
            Console.WriteLine(new string('-', 105));
        }
        ...
        static void AddWatch(List<SmartWatch> list)
        {
            ...
            while (true)
            {
                ...
                catch { Console.WriteLine("[Помилка] Введіть коректне число."); }
            }
            while (true)
            {
                try
                ...
                catch { Console.WriteLine("[Помилка] Введіть ціле число."); }
            }
            while (true)
            {
                try
                ...
                catch { Console.WriteLine("[Помилка] Невірний формат дати."); }
            }
            while (true)
            {
                try
        ...
        static void DemoBehavior(List<SmartWatch> list)
        {
            ...
            if (int.TryParse(Console.ReadLine()!, out int idx) && idx > 0 && idx <= list.Count)
            {
                ...
                Console.WriteLine($"\n--- Тест об'єкта: {selected.ModelName} ---");
